...
var SightControls = function ( object, domElement, targetObject, engine ) {
	...
	this.setPhysicsManager = function(physicsManager){
		this.physicsManager = physicsManager;
		this.physicsEnabled = true;
		this.physicsManager.initControlCamera(this.object);
	}
	...
	this.update = function () {
		...
		return function update() {
			...
			if(scope.isLocked){
				...
				spherical.radius = scope.radius; //Math.max( scope.minDistance, Math.min( scope.maxDistance, spherical.radius ) );
				...
				if(scope.physicsEnabled && scope.moving){
					scope.moving = false;
					scope.physicsManager.updateCamera(offset.clone());
					if(scope.collidingPoint != null){
						position.lerp(scope.collidingPoint.point, 0.1);
						scope.radius = Math.max(scope.collidingPoint.distance, scope.minDistance);
					} else {
						position.copy( scope.target ).add( offset );
						scope.radius = scope.maxDistance;
					}
				} else {
					position.copy( scope.target ).add( offset );
				}
				...
				let refObj = scope.object;//(scope.physicsEnabled) ? scope.physicsManager.cameraBody : scope.object;
				if ( zoomChanged ||
					lastPosition.distanceToSquared( refObj.position ) > EPS ||
					8 * ( 1 - lastQuaternion.dot( refObj.quaternion ) ) > EPS ) {

					scope.dispatchEvent( changeEvent );

					lastPosition.copy( refObj.position );
					lastQuaternion.copy( refObj.quaternion );
					zoomChanged = false;
